...
def make_stage(stage_name, pathId, pathStageId, stageId, blocking, batching, socket, epoll, ngx, net, chunk, recvTm, respTm, cm, 
	criSec, threadLimit, scaleFactor, useHist):
	stage = {}
	...
	assert type(scaleFactor) is float
	stage["scale_factor"] = scaleFactor

	assert type(useHist) is bool
	stage["use_histogram"] = useHist
...
def make_micro_service(servType, servName, bindConn, paths, baseFreq, curFreq):
	service = {}
	...
	assert type(baseFreq) is int
	service["base_freq"] = baseFreq

	assert type(curFreq) is int
	service["cur_freq"] = curFreq
...
def make_serv_inst(servName, servDomain, instName, modelName, sched, machId):
	servInst = {}
	...
	assert type(modelName) is str
	servInst["model_name"] = modelName
...
# workload pattern
# # # # # # # # # #
# policy is given in cmd line
# def make_client(epoch_end_seconds, epoch_kqps, target_tail_lat_ms, monitor_interval_sec):
def make_client(epoch_end_seconds, epoch_kqps, monitor_interval_sec):
	assert type(epoch_end_seconds) is list
	assert type(epoch_kqps) is list
	assert len(epoch_kqps) == len(epoch_end_seconds)
	
	# change to qps
	i = 0
	while i < len(epoch_kqps):
		epoch_kqps[i] = int(epoch_kqps[i]*1000)
		i += 1

	client = {}
	client["epoch_end_sec"] = epoch_end_seconds
	client["epoch_qps"] = epoch_kqps

	# assert type(target_tail_lat_ms) is float or type(target_tail_lat_ms) is int
	# client["target_tail_lat_ms"] = target_tail_lat_ms

	assert type(monitor_interval_sec) is float or type(monitor_interval_sec) is int
	client["monitor_interval_sec"] = monitor_interval_sec
