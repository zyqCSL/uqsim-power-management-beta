...
def make_micro_service(servType, servName, bindConn, paths, baseFreq, curFreq):
	...
	assert type(baseFreq) is float
	service["base_freq"] = baseFreq

	assert type(curFreq) is float
...
def make_client(epoch_end_seconds, epoch_kqps, target_tail_lat_ms, monitor_interval_seconds):
	...
	# change to us
	i = 0
	while i < len(epoch_end_seconds):
		epoch_end_seconds[i] = int(epoch_end_seconds[i]*1000000)
		i += 1
	...
	client = {}
	client["epoch_end_us"] = epoch_end_seconds
	client["epoch_qps"] = epoch_kqps

	assert type(target_tail_lat_ms) is float
	client["target_tail_lat_us"] = target_tail_lat_ms * 1000

	assert type(monitor_interval_seconds) is float
	client["monitor_interval_us"] = monitor_interval_seconds * 1000000
